/* Assume we have Stock table contains all details about stocks,
once u order first it has to check whether qty is available or not 
if qty is available then it has to move to the orders table and 
qty to be updated in stock table.Amt to be added to the Amount table */
...
declare @stockid int,
   ...
   set @flag=1
   select @stockid=StockId,@qtyord=QtyOrd from inserted
   select @qtyavail=Quantity,@stockname=StockName,@price=Price from Stock where StockID=@StockID 
   ...
   Go
	...
	Insert into Orders(StockID, QtyOrd) values(3,10) 
	GO
   select * from Orders
   GO
